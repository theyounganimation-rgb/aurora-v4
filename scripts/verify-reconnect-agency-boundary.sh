...
schedule_start=$(rg -n '^[[:space:]]*private func scheduleReconnect\(after error: Error\)' "$SOURCE" | cut -d: -f1)
schedule_end=$(rg -n '^[[:space:]]*private func scheduleSessionRefresh\(' "$SOURCE" | cut -d: -f1)
...
clear_start=$(rg -n '^[[:space:]]*private func clearPerConnectionTurnState\(\)' "$SOURCE" | cut -d: -f1)

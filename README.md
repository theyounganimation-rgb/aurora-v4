...
the owner's voice
  ...
  -> delegate_task for ordinary work, or codex_project_chat for an explicitly named Codex project/task
...
- Realtime remains Aurora's foreground mind—voice, relationship, memory, personality, and intent. It handles conversation and her private memory/relationship functions itself. Every ordinary request to act outside the conversation—simple Mac control, apps, media, browser work, Notes, Calendar, mail, research, coding, or a novel multi-step goal—becomes one strict `delegate_task` proposal. The sole exception is an explicit request to browse, select, create, leave, or message a named Codex project/task: `codex_project_chat` resolves that resource, persists focus, and relays the finalized owner message without an added task prompt. Both proposals are schema-validated and causally bound to the finalized direct owner turn; deterministic code never reparses wording with a phrase list.
- Explicit Codex focus reuses the selected persistent task across turns and survives Rest/relaunch. Aurora can list the projects currently registered in Codex Desktop, include projects with no tasks, honor explicit Desktop task assignments, group descendant working directories under their longest project root, choose an existing task or defer creation of a new one until its first message, and recover terminal output for voice. A new task in a multi-root project uses Desktop's first registered root. Authorizations bind the exact project root, task cwd/ID, focus generation, owner turn, request ID, message, and expiration. Relays are serialized and idempotent, so reconnect delivery cannot duplicate a message and an older relay cannot resurrect a focus the owner already left. Aurora reads this registry only as a bounded compatibility bridge and never edits Codex Desktop's private project assignments.
- Delegated work runs through one persistent Codex app-server authenticated by the existing ChatGPT subscription sign-in. Codex is Aurora's private hands and feet, named Osiris internally, never a second voice or personality. The task returns immediately so Aurora keeps listening. Corrections steer the exact active turn, cancellation interrupts and drains that exact task, and Rest drains all session-bound work. Each contextual operation is appended to a bounded, tamper-evident recent-effect ledger rather than overwriting the task's original goal; completion truth comes only from the exact executor status plus a trusted turn-bound receipt. Bounded terminal status returns to Realtime only while the authorizing session is still awake. Raw reasoning, commands, screenshots, logs, prompts, and authorization objects never cross into Aurora's speech.
